...
describe('/api/user Tests', () => {
  ...
  it('should return 401 for invalid token', async () => {
    ...
    if (res) {
      ...
      expect(responseJson).toEqual({ error: "Unauthorized. Invalid or missing token." });
    } else {
      throw new Error('Response is undefined');
    }
  });
  ...
  it('should return 500 if there is an error fetching the user', async () => {
    ...
    vi.spyOn(UserService, 'getUserByIDService').mockRejectedValue(new Error("An error occurred"));
    ...
    if (res) {
      ...
      // Check that the response contains the error message
      expect(responseJson).toEqual({ error: "An error occurred while fetching the user" });
    } else {
      throw new Error('Response is undefined');
    }
  });
