...
import { checkToken,isAdmin } from "@/lib/midlleware/auth";
  ...
  export const POST= async (req:NextRequest) => {
    ...
    try {
      ...
      console.error(error);
      return  NextResponse.json({ error: "Error in adding new category. Please try again after some time" });
    }
  };
  ...
  export const PATCH = async (req: NextRequest) => {
    ...
    try {
      ...
      console.error(error);
      return  NextResponse.json({ error: "Error in updating ctegories. Please try again after some time." });
    }
  };
